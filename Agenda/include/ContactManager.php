...
class ContactManager {
    ...
    public function updateContact($id, $name, $surname, $email) {
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error al actualizar el contacto: " . $stmt->error);
        }

        $stmt->close();
    }

    public function updateContactNumber($contactId, $number) {
        $numberId = $this->getNumberId($number);

        if ($numberId === null) {
            $numberId = $this->insertNumber($number);
        }
        $this->updateContactNumberRelation($contactId, $numberId);
    }

    private function getNumberId($number) {
        $sql = "SELECT id FROM number WHERE number=?";
        $stmt = $this->conn->prepare($sql);
    ...
    private function insertNumber($number) {
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error al insertar el número: " . $stmt->error);
        }
    ...
    private function updateContactNumberRelation($contactId, $numberId) {
        $sql = "UPDATE contact_number SET number_id=? WHERE contact_id=?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("ii", $numberId, $contactId);

        if (!$stmt->execute()) {
            throw new Exception("Error al actualizar la relación número-contacto: " . $stmt->error);
        }
